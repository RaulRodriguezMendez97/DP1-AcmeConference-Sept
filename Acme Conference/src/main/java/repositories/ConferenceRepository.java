...
public interface ConferenceRepository extends JpaRepository<Conference, Integer> {
	...
	//Raul
	@Query("select c from Conference c where now() <= c.submissionDeadline and c.finalMode=1")
	...
	@Query("select c from Conference c where now() < c.startDate and c.finalMode=1 and c.admin.id = ?1")
